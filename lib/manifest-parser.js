...
function splitNameAndVersion(nameAndVersion) {
  // e.g. tweetnacl@^0.14.3, tweetnacl@~0.14.0
  var possibleMultipleRequirements = nameAndVersion.split(', ');
  // only use first requirement's name -- is it possible for multiple requirements with different names to resolve to same dep?
  ...
  var version = versions.join(' '); // comaparator intersection (https://classic.yarnpkg.com/lang/en/docs/dependency-versions/#toc-intersections)
